...
namespace FileReceiver
{
    public class AESEncryptionStrategy : IEncryptionStrategy
    {
        ...
        public AESEncryptionStrategy(string keyInput,string ivInput)
        {
            // Example of using a fixed 16-byte key and IV.
            key = Encoding.UTF8.GetBytes(keyInput);  // 16-byte key (128-bit)
            iv = Encoding.UTF8.GetBytes(ivInput);   // 16-byte IV
        }
