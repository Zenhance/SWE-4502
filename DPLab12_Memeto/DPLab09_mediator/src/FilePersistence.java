...
public class FilePersistence {
    ...
    public static void saveToFile(String filePath, List<EditorMemento> history) {
        try (ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(filePath))) {
            out.writeObject(history.getLast());  // Write the list of mementos to the file
    ...
    public static List<EditorMemento> loadFromFile(String filePath) {
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(filePath))) {
            ...
            List<EditorMemento> rtrnList = new ArrayList<>();
            rtrnList.add( (EditorMemento) in.readObject());
            return rtrnList;
        } catch (IOException e) {
